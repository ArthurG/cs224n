...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight =  nn.init.xavier_uniform_(nn.Parameter( torch.randn(self.n_features * self.embed_size, hidden_size) ))
        #self.embed_to_hidden_weight =(self.embed_to_hidden_weight )
        self.embed_to_hidden_bias  = nn.init.uniform_(nn.Parameter(torch.randn(hidden_size)))

        self.dropout = nn.Dropout(dropout_prob)
    
        self.hidden_to_logits_weight = nn.init.xavier_uniform_(nn.Parameter(torch.randn(hidden_size, n_classes)))
        self.hidden_to_logits_bias = nn.init.uniform_(nn.Parameter(torch.randn(n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        tensor_list = []
        for i in w:
            row_vec = torch.index_select(self.embeddings, 0, i).view(self.n_features * self.embed_size)
            tensor_list.append(row_vec)
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.add(torch.matmul( self.embedding_lookup(w), self.embed_to_hidden_weight), self.embed_to_hidden_bias))
        h = self.dropout(h)
        logits = torch.add(torch.matmul(h, self.hidden_to_logits_weight), self.hidden_to_logits_bias)
